...
public class FU {
	...
	static TextView.OnEditorActionListener OnEditorDone;
	public static TextView.OnEditorActionListener getOnEditorDone() {
		if(OnEditorDone==null) {
			OnEditorDone = (v, actionId, event) -> {
				if(actionId == EditorInfo.IME_ACTION_DONE ||actionId==EditorInfo.IME_ACTION_UNSPECIFIED) {
					((View)v.getTag()).performClick();
					return true;
				}
				return false;
			};
	...
	public static EditText buildStandardTopETDialog(Context c, boolean showKeyBoard, int gravity) {
		final ViewGroup dv = (ViewGroup) LayoutInflater.from(c).inflate(R.layout.fp_edittext, null);
		...
		//final Dialog dd = new GoodKeyboardDialog(c);
		final Dialog dd = new GoodKeyboardDialog(new ContextThemeWrapper(c, R.style.Base_Theme_AppCompat_Dialog_Alert));
		dd.requestWindowFeature(Window.FEATURE_NO_TITLE);
		dd.setContentView(dv);
		etNew.setOnEditorActionListener(FU.getOnEditorDone());
		etNew.setTag(btn_Done);
		etNew.setSingleLine(false);
		btn_Done.setTag(btn_SwicthFolderCreation);
		btn_SwicthFolderCreation.setTag(dd);
		if(true) {
		...
		//win.setBackgroundDrawableResource(R.drawable.popup_shadow_s);
